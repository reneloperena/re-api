...
export async function authenticateToken(token: string): Promise<Context> {
  const decoded = await admin.auth().verifyIdToken(token);

  const userId = decoded.uid;
  const clientId = decoded.client_id || ""; // customize if needed
  const role = (decoded.role as UserRole) || UserRole.Viewer;
